...
`ion-chain-gas-flow` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for gas constraint solving, using bounded scenario files and conflict explanations.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 3.
